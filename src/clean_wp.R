...
INPUT_PATH <- file.path(BASE_PATH, "anonym_datas_output_with_problem")

# A kimeneti állományok ide fognak kerülni:
OUTPUT_PATH <- file.path(BASE_PATH, "datas_wp")
...
LOG_FILE <- file.path(BASE_PATH, "clean_wp.R.log")
...
vcdatas <- load_file(file.path(INPUT_PATH, "vcenter_datas_wp.RData"))
...
################################################################################
# Mentünk:
save_data(vcdatas, "vcenter_datas_cleaned_wp", tsv=FALSE)
