...
INPUT_PATH <- file.path(BASE_PATH, "anonym_datas")
...
LOG_FILE <- file.path(BASE_PATH, "aggregator.R.log")
...
INPUT_RFILE <- paste(OUTPUT_PATH, "vcenter_datas.RData", sep="/")
...
for (INPUT_FILE in INPUT_LOG_FILES) {
    ...
    tryCatch(load(file=file.path(INPUT_RFILE)),
             error=function(e) {
                 ...
                 if (charmatch("Error in readChar(con, 5L, useBytes = TRUE): cannot open the connection\n",e)) {
                     ...
                     vcdatas <- data.frame()
                     save_data(data=vcdatas, filename="vcenter_datas", 
                               tsv=FALSE)
    ...
    save_data(data=vcdatas, filename="vcenter_datas", tsv=FALSE)
    
}

save_data(data=vcdatas, filename="vcenter_datas", tsv=TRUE)
