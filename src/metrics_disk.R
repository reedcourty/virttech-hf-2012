...
stlatency_plotter <- function(datas, ST, ET) {
    ...
    plot <- ggplot() +
        ...
        geom_line(data=vcdatas_limit,
                  aes(x = timestamp,
                      y = storageadapter.totalwritelatency.average_vmhba33,
                      colour="2")) +
        geom_point(data=vcdatas_limit,
                   aes(x = timestamp, 
                       y = storageadapter.totalwritelatency.average_vmhba33, 
        ...
        scale_colour_manual(values=c("#4E9A06", "#A40000", "#204A87"), 
                            name="Teljes írási késleltetések",
...
unique(vcd$storageadapter.totalwritelatency.average_vmhba33)
unique(vcd$storagepath.totalwritelatency.average_iqn.com.vmware.host01)

vcd$timestamp[vcd$storageadapter.totalwritelatency.average_vmhba33==63]

ST <- "2012-09-03 04:00:00"
ET <- "2012-09-03 04:10:00"
