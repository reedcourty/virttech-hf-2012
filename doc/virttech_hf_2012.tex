...
\begin{figure}[ht!]
  \centering
  \subfloat[host-01 (Korreláció: $0,999999734514543$)]{\label{fig:corrgram-host-01}\includegraphics[width=0.40\textwidth]{figures/corrgram-host-01-0999999734514543.png}}                
  \subfloat[host-02 (Korreláció: $0,999999692814215$)]{\label{fig:corrgram-host-02}\includegraphics[width=0.40\textwidth]{figures/corrgram-host-02-0999999692814215.png}}
...
\begin{wrapfigure}{r}{0.60\textwidth}
  ...
  \end{center}
  \caption{A kernel, eszköz és teljes késleltetés viszonya (Forrás: \cite{link:TDK}) \label{fig:tdk}}
\end{wrapfigure}
...
A tárolórendszerünk teljesítményét jellemezhetik a \textit{storage path} és \textit{storage adapter} metrikák. Ezek vizsgálatát, és a lemezmetrikákkal való kapcsolatát láthatjuk \aref{fig:disk_metrics_storage_latency-01}., \ref{fig:disk_metrics_storage_latency-02}. és \ref{fig:disk_metrics_storage_latency-03}.~ábrákon.

\Aref{fig:disk_metrics_storage_latency-01}.~ábra a teljes mérési időintervallum alatt összegyűjtött teljes írási késleltetéseket mutatja a path, adapter és lemez esetében. 
...
Érdemi információhoz inkább \aref{fig:disk_metrics_storage_latency-02}. és \aref{fig:disk_metrics_storage_latency-03}.~ábra esetében jutunk. Ezekről megállapítható, hogy az adapter és a lemez késleltetés mindig megegyezik (a zöld és kék görbe együtt fut a mérés ideje alatt).
...
%###############################################################################
%#
%#                    A net.transmitted.average metrika:
%#
...
\Aref{fig:net_transmitted_average-01}.~ábrán a teljes mérés ideje alatt, míg \aref{fig:net_transmitted_average-02}.~ábrán egy rövidebb intervallumon rögzített átviteli sebesség alakulása figyelhető meg.
...
Ha külön kezeljük a két hálózati eszközt és a szummázott értékeket, akkor \aref{fig:net_transmitted_average_dev}.~ábrát kapjuk, amiről megállapítható, hogy a szummázott eredmény látszólag jól visszaadja a két eszköz értékeit (hiszen a mért értékeket mutató sárga görbét mindenütt fedi a számított kék görbe).
...
Viszont ha megnézzük a mért és számított átviteli-sebességeket, akkor \aref{fig:net_transmitted_average_diff-01}. és \aref{fig:net_transmitted_average_diff-02}.~ábrákról megállapíthatjuk, hogy az eltérés 0 és 1 kB/s között ugrál. 
...
Azt, hogy mekkora arányban vannak eltérések, \aref{fig:net_transmitted_average_diff-dist}.~ábra mutatja meg. Mint látható, ugyan nagyobb számban egyezik meg a mért és a két eszközön mért sebességek számított összege, de sok esetben előfordul 1 kB/s-os eltérés.
...
%#
%#
%#                    Néhány CPU-val kapcsolatos metrika:
%#
\begin{table}[ht]
...
%#
%#
%#                    Néhány lemezzel kapcsolatos metrika:
%#
\begin{table}[ht]
...
%#
%#
%#                    Néhány hálózattal kapcsolatos metrika:
%#
\begin{table}[ht]
